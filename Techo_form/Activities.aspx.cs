            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (tb_DescriptionActiv.Text.Length < 5)
                            {
                            ...
                            {
                                if(ddl_ActivityType.SelectedValue == -1)
                                {
                                lbl_output_Form.Text = "Escoga un tipo de actividad, si la actividad que usted gusta ingresar contacte a soporte tecnico o ponga "Otros".";
                                lbl_output_Form.BackColor = System.Drawing.Color.LightPink;
                                lbl_output_Form.ForeColor = System.Drawing.Color.DarkRed;
                                lbl_output_Form.Visible = true;
                                }
                                ...
                                {
                                    if(ddl_CountryActiv.SelectedValue == -1)
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                if(Convert.ToInt32(tb_capacityactiv.Text) <= 0)
                                                {
                                                ...
                                                else
                                                {
                                                    try
                                                    {
                                                        udf.Execute_Non_Query(activity.Insert_New_Activity(tb_nameactiv.Text, Convert.ToInt32(ddl_Cityactiv.SelectedItem.Value)
                                                            , Convert.ToInt32(ddl_Coordinatoractiv.Text), Convert.ToDouble(tb_Workhours.Text), tb_DescriptionActiv.Text
